...
**Endpoint:**     `https://vault.wiremaze.internal:8200` (produção) · `http://127.0.0.1:8200` (dev)
**Auth:**         AppRole (`auth/approle/role/wiremaze-<subagent>`)
...
- `ssh/sign/wmz-srv-role` — certificados SSH efémeros (TTL=15m) para servidores
- `ssh/sign/wmz-ir-role` — certificados SSH efémeros para IR

### AppRoles activos

| AppRole | Subagent | TTL | Max TTL |
|---------|----------|-----|---------|
| `wiremaze-monitor` | `wiremaze-monitor-01` | 30m | 1h |
| `wiremaze-ir` | `wiremaze-ir-saas-01` | 15m | 1h |
| `wiremaze-tenant` | `wiremaze-tenant-01` | 15m | 30m |
| `wiremaze-srv` | `wiremaze-srv-saas-01` | 15m | 30m |
| `wiremaze-deploy` | `wiremaze-deploy-01` | 15m | 30m |
| `wiremaze-compliance` | `wiremaze-compliance-01` | 30m | 1h |
| `wiremaze-cowork-reporting` | Cowork `ai-rep-01` | 60m | 2h |
...
Ops privilegiadas **falham-fechadas**. Sem fallback inseguro. Os **doctors do plugin continuam a correr** (allowlist) — corre `/wiremaze-vault-doctor` primeiro para identificar o que está em down. Recovery: `vault token renew` ou re-login com `role_id` + `secret_id`. Em dev, `export VAULT_TOKEN=dev-only-root`.
...
for policy in $(grep -oE 'wiremaze-[a-z]+' vault-policies.hcl | sort -u); do
...
`wiremaze-monitor-01` Wazuh+Fortigate+Zabbix · `wiremaze-ir-saas-01` IR multi-tenant · `wiremaze-tenant-01` isolamento · `wiremaze-srv-saas-01` servidores Rails nativos · `wiremaze-deploy-01` release gate (Capistrano) · `wiremaze-compliance-01` compliance
...
- Releases têm gate. Nenhum `cap production deploy` sem `/wiremaze-release-gate` aprovado e canary multi-tenant.
...
**Operação:**
`/wiremaze-saas-health` · `/wiremaze-tenant-audit <municipio>` · `/wiremaze-incident-spread <id>` · `/wiremaze-release-gate <release>` · `/wiremaze-cliente-dossier <municipio>` · `/wiremaze-compliance-snapshot`

**Diagnóstico (doctors):**
`/wiremaze-stack-doctor` (global) · `/wiremaze-vault-doctor` · `/wiremaze-ollama-doctor`

## Skills (em `skills/`)
`wiremaze-tenant-isolation` · `wiremaze-saas-monitoring` (Wazuh+Fortigate+Zabbix) · `wiremaze-ir-multitenant` · `wiremaze-release-safety` · `wiremaze-compliance-provider` · `wiremaze-cliente-dossier`
...
- `WMZ.POL.SEC.001` — Política SecOps
- `WMZ.ARQ.SEC.002` — Arquitectura (servidores nativos Rails)
- `WMZ.PRC.AUD.004` — Auditoria
- `WMZ.PRC.IRT.005` — IR multi-tenant
- `WMZ.MTZ.SEC.006` — RACI + CTRL-W-*
