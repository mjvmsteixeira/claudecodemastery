...
# /wire-secops-bootstrap [--plan | --apply]

Provisiona o conteúdo Wire-specific no Vault assumindo que a infra genérica já existe (`/wire-vault-bootstrap` correu antes).
...
case "$MODE" in
  --plan|--apply) ;;
  *) echo "Uso: /wire-secops-bootstrap [--plan | --apply]" >&2; exit 1 ;;
esac
```

## Passo 2 — Runtime-find de lib/vault-env.sh do wire-base

```bash
VAULT_ENV=""
for path in \
  "${HOME}/.claude/plugins/cache"/*/wire-base/*/lib/vault-env.sh \
  "${HOME}/.claude/plugins/cache"/*/*/wire-base/*/lib/vault-env.sh \
...
if [ -z "$VAULT_ENV" ]; then
  cat >&2 <<'EOF'
wire-base não detectado. wire-secops-bootstrap depende de wire-base.
...
```bash
if ! vault_ready; then
  echo "Vault inacessível ou sealed. Corre /wire-vault-doctor." >&2
  exit 1
fi
...
# Verifica que approle/transit/ssh já existem (caso contrário manda para wire-vault-bootstrap)
...
if [ -n "$MISSING" ]; then
  ...
  echo "Corre /wire-vault-bootstrap --plan && /wire-vault-bootstrap --apply primeiro." >&2
  exit 1
fi
```
...
if [ ! -f "$HCL_FILE" ]; then
  echo "vault-policies.hcl não encontrado em $HCL_FILE — reinstala wire-secops." >&2
  exit 1
fi
```
...
```bash
echo "── Plano: /wire-secops-bootstrap ──"
echo
printf "%-3s %-14s %-26s %s\n" "" "Tipo" "Nome" "Status"
echo "──────────────────────────────────────────────────────────────────────"
...
if [ "$MODE" = "--plan" ]; then
  ...
  if [ "$CREATE" -gt 0 ] || [ "$ROTATE" -gt 0 ]; then
    echo "Para executar: /wire-secops-bootstrap --apply"
  else
    echo "Nada a fazer."
  fi
  exit 0
...
echo
echo "✓ /wire-secops-bootstrap --apply completo."
echo
echo "Próximos passos:"
echo "  /wire-vault-doctor      # validar findings resolvidos"
