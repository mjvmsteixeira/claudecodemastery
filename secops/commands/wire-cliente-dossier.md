...
Activa a skill `wire-cliente-dossier` e os subagentes `wire-tenant-01`, `wire-monitor-01`, `wire-compliance-01`.

Sequência:
1. Resolve `$ARGUMENTS` para `tenant_id` UUID.
2. Recolhe automaticamente (em paralelo):
   - **Produtos e versões** (de `wire-tenant-01`).
   - **SLA realizado 12m + histórico de incidentes** (de `wire-monitor-01`).
   - **Compliance específica** — DPIA, Anexo II, sub-subcontratantes, residência de dados (de `wire-compliance-01`).
