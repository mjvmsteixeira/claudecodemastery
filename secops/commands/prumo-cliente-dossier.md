...
Activa a skill `prumo-cliente-dossier` e os subagentes `prumo-tenant-01`, `prumo-monitor-01`, `prumo-compliance-01`.

Sequência:
1. Resolve `$ARGUMENTS` para `tenant_id` UUID.
2. Recolhe automaticamente (em paralelo):
   - **Produtos e versões** (de `prumo-tenant-01`).
   - **SLA realizado 12m + histórico de incidentes** (de `prumo-monitor-01`).
   - **Compliance específica** — DPIA, Anexo II, sub-subcontratantes, residência de dados (de `prumo-compliance-01`).
