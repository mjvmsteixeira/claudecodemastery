...
Executa diagnóstico completo do Ollama local que sustenta o hook `pre-tool-second-opinion.sh` do plugin prumo SecOps.
...
Estas vêm do `~/.prumo/secops.conf` ou env vars exportadas. Se ausentes, usa defaults.

## Cadência sugerida

- **Início do turno**, após re-login AppRole Vault (ver `/prumo-vault-doctor`) e antes de operar.
...
- **Não testa todos os modelos** que podem estar em uso (ex: se houver modelos para outros plugins). Foca-se no que o `prumo-secops` depende.
