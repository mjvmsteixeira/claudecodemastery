...
Chama internamente o playbook do `/prumo-vault-doctor`. Resumo só com verdicto final + 1-3 issues críticas se existirem.

### 2. Ollama (crítico para ops destrutivas)

Chama internamente o playbook do `/prumo-ollama-doctor`. Sem Ollama, hook `pre-tool-second-opinion.sh` bloqueia.

### 3. Wazuh quick-check (crítico para audit)

```bash
: "${PRUMO_WAZUH_HOST:?PRUMO_WAZUH_HOST não definido. Export antes de correr stack-doctor.}"
...
- **WARN** se inacessível → `prumo-saas-monitoring` skill perde uma das 3 fontes.
...
Próximos passos:
  1. /prumo-vault-doctor    (detail completo Vault)
  2. /prumo-ollama-doctor   (detail completo Ollama)
...
O `prumo-stack-doctor` ressalta no topo o que **podes** e o que **não podes** fazer no estado actual. Exemplo:

```
✓ podes correr: /prumo-saas-health (local), /prumo-tenant-audit (sem cross-tenant)
✗ não corras:   /prumo-incident-spread (precisa Fortigate)
```
