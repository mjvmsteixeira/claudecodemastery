...
> Template referenciado pela skill `wire-ir-multitenant`. Baseado em NIST SP 800-61r2 §3
...
Obrigatório no topo de cada timeline (ficheiro `${WIRE_FORENSICS_DIR}/wire-<incident-ID>/timeline.md`):
...
|----------------------|---------------|-----|-------------------------------------------------------------------------|--------------------|----------------|
| 2026-05-19T08:32:14Z | Wazuh         | S2  | Alert rule_id 100012 — PSQL auth fails > threshold em wirePAPER (5 ten) | wire-monitor-01    | investigating  |
| 2026-05-19T08:35:01Z | human         | S2  | Pivot → S1: pattern matches CVE-2026-XXXX explotation                   | carlos.rocha       | escalated      |
| 2026-05-19T08:40:55Z | Fortigate     | S1  | Block applied: src IP 1.2.3.4 rate-limit em todas as regiões            | wire-srv-saas-01   | contained      |
...
Toda a maquinaria Wire (Wazuh, Fortigate, Zabbix, Vault, servidores Rails) corre em UTC. NTP sync verificado em `/wire-stack-doctor`. Se um actor humano em Portugal escrever "10:32 da manhã", o scribe converte para UTC antes de registar.
...
| UTC                  | source      | sev | action                                                                   | actor            | status        |
|----------------------|-------------|-----|--------------------------------------------------------------------------|------------------|---------------|
| 2026-05-19T08:32:14Z | Wazuh       | S2  | Rule 100012 — PSQL auth fails 50+ em wirePAPER (5 tenants distintos)     | wire-monitor-01  | investigating |
| 2026-05-19T08:33:50Z | Fortigate   | S2  | IPS hit srcip 1.2.3.4 sig "rack-attack-bypass-attempt" 14 ocorrências    | wire-monitor-01  | investigating |
...
| 2026-05-19T08:40:55Z | Fortigate   | S1  | Block src 1.2.3.4 + /24 aplicado nas 3 regiões perímetro                 | wire-srv-saas-01 | contained     |
...
| 2026-05-19T09:48:01Z | capistrano  | S1  | cap production deploy:rollback wirePAPER concluído                       | wire-deploy-01   | recovering    |
| 2026-05-19T10:15:00Z | Zabbix      | S1  | Triggers wirePAPER back to OK em 5/5 tenants                             | wire-monitor-01  | recovering    |
...
A skill `wire-ir-multitenant` invoca este template no início de um incidente para criar `${WIRE_FORENSICS_DIR}/wire-<id>/timeline.md` e em cada evento subsequente para adicionar rows. Esperar como output: timeline auto-actualizada após cada decisão ou alert agregado. O user mantém controlo total — qualquer row pode ser editada manualmente, mas alterações pós-closure exigem `audit-note` na própria timeline.
