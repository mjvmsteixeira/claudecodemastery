...
> Template referenciado pela skill `wire-compliance-provider`. Baseado em Directiva (UE)
...
| Tratamento de incidentes | Art. 14 §1 alínea c) | Skill `wire-ir-multitenant` + WIRE.PRC.IRT.005 |
...
Para a Wire isto traduz-se directamente em **S1 e maioria dos S2** da matriz interna (ver `wire-ir-multitenant/references/severity-matrix.md`).
...
| CTRL-W-C-025 | Notificação CNCS em ≤ 24h documentada | Art. 23 §1 | Playbook em `wire-ir-multitenant` + log envios |
...
Implementação: playbook `wire-ir-multitenant` invoca template `distribuicao-classificacao.md` que disparam notificação CNCS para todo o S1 e S2 acima do threshold. Submissão via portal CNCS-CERT.PT. Confirmação de receção arquivada em `secret/data/compliance/notificacoes-cncs/<incident-id>/`.
...
A skill `wire-compliance-provider` invoca este template em `/wire-compliance-snapshot`, em prep de auditorias NIS2, em resposta a questionários de cliente que peçam mapeamento DL 20/2025, ou em notificação CNCS durante incidente. Esperar como output: tabela cruzada CTRL-W × artigo + notificações pré-preenchidas com placeholders concretos da sessão. O user revê e submete — a skill nunca envia ao CNCS directamente.
