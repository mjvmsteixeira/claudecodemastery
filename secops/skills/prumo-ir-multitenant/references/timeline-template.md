...
> cadeia de custódia) e do <ORG>.PRC.IRT.005. Validar com o Coordenador SecOps antes de servir de
> base a peça formal.
...
1. **UTC sempre**, formato `YYYY-MM-DDTHH:MM:SSZ`. Os servidores `os produtos do inventário` estão em fusos e horários de verão diferentes; misturar locais torna a correlação impossível.
...
4. **Cada linha aponta para evidência** — `rule_id` do Wazuh, ID de sessão do Fortigate, caminho do artefacto em `${PRUMO_FORENSICS_DIR}/<prefixo>-<incident-ID>/`, hash SHA-256.
...
Produtos:         <produto> (Rails <v>), <outro-produto> (Rails <v>)
...
|---|-----|-------|-----------------|------------|---------------|-----------|
| 1 | `2026-…T14:02:11Z` | Wazuh | `rule_id=5710` · 40 auth falhadas para `<prefixo>-srv-03` | Fortigate: sessões da mesma origem 14:01:47Z | *Hipótese:* brute-force a partir do exterior | `evidence/wazuh-5710.json` · `sha256:…` |
| 2 | `2026-…T14:03:02Z` | Fortigate | IPS bloqueou origem `<IP>` | — | Perímetro conteve | `evidence/fgt-ips-…log` |
| 3 | `2026-…T14:22:40Z` | Wazuh | Acesso bem-sucedido a `<prefixo>-srv-07` | **sem correspondência Fortigate** | *Hipótese:* movimento lateral. Sobe a S2 pela regra do sinal ausente | `evidence/wazuh-…json` |
...
Toda a evidência em `${PRUMO_FORENSICS_DIR:-$HOME/forensics}/<prefixo>-<incident-ID>/`, com hash à recolha:

```bash
find "${PRUMO_FORENSICS_DIR:-$HOME/forensics}/<prefixo>-<ID>/evidence" -type f -exec shasum -a 256 {} \; \
  > "${PRUMO_FORENSICS_DIR:-$HOME/forensics}/<prefixo>-<ID>/MANIFEST.sha256"
```
