...
**Skill:** `prumo-tenant-isolation` · **Versão:** v0.4.0 · **Última actualização:** 2026-05-19

> Template referenciado pela skill `prumo-tenant-isolation`. Queries SQL canónicas para validar
...
Toda query executada em sessão de audit é logada em `${PRUMO_FORENSICS_DIR}/<incident-id>/queries.sql`
com timestamp UTC + actor + output sanitizado. As próprias queries de validação NÃO devem ser
executáveis sem `PRUMO_APPROVE=N1` (operação read-only é N1, mas continua audit-tracked).
...
A skill `prumo-tenant-isolation` invoca este template em `/prumo-tenant-audit <municipio>` para executar bateria de queries e gerar evidência. Esperar como output: tabela CTRL-W-T-* com PASS/WARN/FAIL por query + queries originais arquivadas + sentinel baselines actualizados. Em ambiente de produção a sessão exige `PRUMO_APPROVE=N1` mesmo para queries read-only (audit por defeito).
