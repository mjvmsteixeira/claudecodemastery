...
> Template referenciado pela skill `prumo-tenant-isolation`. Vista consolidada de todos os
...
A skill `prumo-tenant-isolation` invoca este template em `/prumo-tenant-audit --painel` (vista global) ou `/prumo-tenant-audit <municipio>` (drill-down individual). Esperar como output: ASCII formatado pronto para revisão em terminal ou export. O user escolhe granularidade — global, per-control, per-tenant, ou tendência. Histórico arquivado automaticamente para diffs entre runs.
