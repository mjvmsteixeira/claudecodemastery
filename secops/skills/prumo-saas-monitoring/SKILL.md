---
name: prumo-saas-monitoring
description: Monitorização contínua e triagem de alertas da stack SaaS multi-tenant (produtos declarados em ~/.prumo/org.json) sobre servidores nativos Ruby on Rails. Combina três fontes — **Wazuh** (SIEM/auditor), **Fortigate** (firewall perímetro) e **Zabbix** (monitorização activa de hosts/serviços). Inclui auditoria da própria monitorização: hosts sem agente Zabbix, equipamentos sem template adequado, alertas silenciosos, triggers obsoletos, correcções e melhorias propostas. Usa esta skill para "/prumo-saas-health", status da plataforma, triagem de alerta, correlação Wazuh ↔ Fortigate, auditoria de cobertura Zabbix, e diagnóstico inicial de degradação por produto ou município. Dispara em "como está a stack", "audita o Zabbix", "que hosts ficaram fora de monitorização", "este alerta Wazuh bate com o que viu o Fortigate", "qual produto está degradado", "SLA do município X", "porque está lenta a API".
---

# Monitorização e Auditoria de Monitorização

## Pré-requisitos

- `prumo-base` instalado (para `V()` de `lib/vault-env.sh` — abstracção native/docker do CLI Vault).
- Vault acessível com AppRole `<prefixo>-monitor` (read em `secret/data/observability/*`).
...
WAZUH_HOST="${PRUMO_WAZUH_HOST:-${WAZUH_HOST:-wazuh-manager.$(prumo_org domain)}}"
...
1. **Health checking** — saúde dos produtos do inventário e da infraestrutura subjacente.
...
O subagente `prumo-monitor-01` opera sobre estas fontes em modo read-only (AppRole `<prefixo>-monitor`, TTL=30m).

**Endpoints (override via env):**
- Wazuh manager: `${PRUMO_WAZUH_HOST:-${WAZUH_HOST:-wazuh-manager.$(prumo_org domain)}}` (3-tier fallback — namespace canónico → legacy → default composto do domínio).
- Fortigate: `${PRUMO_FORTIGATE_HOST:-fortigate.$(prumo_org domain)}`.
- Zabbix API: `${ZABBIX_URL:-https://zabbix.$(prumo_org domain)/api_jsonrpc.php}`.

## Severidades

| Sev | Critério | Janela alvo de triagem |
|-----|----------|------------------------|
| **P1** | Produto indisponível para ≥1 município OR taxa de erro >5% sustentada por 5min OR vazamento cross-tenant suspeito | Imediata |
...
- Pergunta de saúde geral → executa o painel ASCII (ver "Painel `/prumo-saas-health`").
...
- Que produtos do inventário? Mais do que um aponta para camada infra (BD, rede, Vault, Fortigate).
...
- P1 → escalada imediata ao IR (`prumo-ir-saas-01`).
- P2 → engenharia do produto afectado, dentro do horário.
...
| **Templates aplicados** | Host de tipo X sem template canónico esperado (ex: nó Rails sem `Template-<ORG>-Rails-Puma`) | Alto |
...
| **Templates por produto** | Cada produto deve ter template dedicado com items específicos (Puma worker count, Rails error rate, lograge tail) | Alto |
...
2. **Inventário esperado.** Cruza com a CMDB da organização (ou inventário Ansible) para identificar hosts que existem mas não estão em Zabbix.
...
6. **Output:** relatório de cobertura Zabbix com propostas concretas:
   ...
   ```
   == <ORG> · Zabbix Coverage Audit · YYYY-MM-DD ==
   
   Hosts inventariados (CMDB):      214
   Hosts em Zabbix:                 207
   Hosts sem agent activo:          3
     - <host-rails-a1> (Pool A) — última heartbeat há 14h
     - <host-rails-b2> (Pool B) — Zabbix agent não responde, host UP
     - db-tenant-cm-XX-01      — host não inventariado em Zabbix
   
   Hosts sem template canónico:     7
     - <host-rails-a3> — falta Template-<ORG>-Rails-Puma
   ...
   Propostas de melhoria:
     1. Reinstalar agent em <host-rails-a1> (firewall a bloquear porta 10050?)
     2. Aplicar Template-<ORG>-Rails-Puma aos 7 hosts identificados
...
- **Trimestral:** revisão de templates contra evolução da stack (versões Rails, novos produtos do inventário).

---

## Painel diário (`/prumo-saas-health`)
...
<produto-1>   99.95%    180      0.04%      0           Rails 6.1 · Pool A
<produto-2>   99.99%    140      0.01%      0           Rails 7.1 · Pool A
<produto-3>   99.92%    220      0.08%      0           Rails 7.2 · Pico 14:32, recuperado
<produto-4>   99.80%    310      0.21%      1           Rails 7.0 · Em análise
<produto-5>   100.00%   95       0.00%      0           Rails 7.1 · Pool B
<produto-6>   99.97%    270      0.03%      0           Rails 7.0 · Pool B
<produto-7>   99.95%    180      0.05%      0           Rails 7.1 · Pool B
<produto-8>   99.98%    160      0.02%      0           Rails 6.1 · Pool B
...
- `references/painel-template.md` — template completo do `/prumo-saas-health`.
- `references/wazuh-rules.md` — IDs de regras Wazuh relevantes por produto.
...
- `<ORG>.ARQ.SEC.002` — Arquitectura técnica (ver `secops/CLAUDE.md`).
