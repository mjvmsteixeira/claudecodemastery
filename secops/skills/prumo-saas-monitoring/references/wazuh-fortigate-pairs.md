...
> Template referenciado pela skill `prumo-saas-monitoring`. Baseado em Fortinet FortiGate
...
A skill `prumo-saas-monitoring` aplica estes 4 pares canónicos.
...
A skill `prumo-saas-monitoring` invoca este template em `/prumo-saas-health` quando há alerta Wazuh sem contexto Fortigate (ou vice-versa), durante triage de IR para validar correlação cruzada, ou em audit periódico das métricas de saúde. Esperar como output: classificação do par (confirmado / red-flag / informativo) + runbook específico aplicável. O user mantém o último julgamento — a sessão sinaliza, humano decide escalation.
