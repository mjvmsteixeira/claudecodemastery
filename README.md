...
| **prumo-secops** | 0.5.0 | SecOps com Agentes IA para a Wire enquanto fornecedora SaaS de eGovernment local (170+ autarquias). 6 agents, 10 commands `/prumo-*` (inclui `/prumo-secops-bootstrap` para provisionar policies + AppRoles + Keychain numa só corrida), 6 skills com **20 templates `references/`** (progressive disclosure), cadeia de hooks PreToolUse/PostToolUse/Stop **funcional** (v0.4.0 corrige hooks que estavam bypassed: approval-gate via `PRUMO_APPROVE`, pii-redact fail-closed). Assume `prumo-base` instalado. |
...
| **prumo-craft** | 0.2.0 | Tooling generativo — `html-plan` (HTML designed em 2 fases com disciplina anti-AI-slop: 8px grid, contraste WCAG AA, real data, 5 surfaces). Standalone, zero deps externas. **Novidade v0.1.0.** Roadmap: `logo-generator` em v0.2.0. |
