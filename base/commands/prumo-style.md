...
```
/prumo-style                 # status: mostra se o bloco existe (projecto + user) e versão
...
build_block() {
  cat <<EOF
<!-- prumo-style BEGIN v${STYLE_VERSION} -->
...
if [ "$ACTION" = "on" ]; then
  ...
  prumo_log prumo-base style "on $SCOPE v${STYLE_VERSION}"
fi
