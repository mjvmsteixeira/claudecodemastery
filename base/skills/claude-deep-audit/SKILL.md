...
Se `lib/wmz-common.sh` estiver disponível, o relatório final pode ser logado em `~/.wmz/log/claude-deep-audit.log` via `wmz_log` para histórico cross-session.

## Ver também

- `mempalace-doctor` (skill irmã) — diagnóstico do **tool MemPalace**, não da configuração Claude Code.
- `/wiremaze-vault-doctor` (em wiremaze-secops) — diagnóstico do **servidor Vault** (HA, seal, audit, AppRoles).
