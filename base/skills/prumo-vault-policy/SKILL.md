...
Skill-trigger que delega para `/prumo-vault-policy <nome>`. Gera um template HCL parametrizado para uma policy Vault nova, escrito em `$VAULT_HOME/policies/<nome>-policy.hcl` para o utilizador rever antes de aplicar.
...
Inferir os parâmetros da intenção do utilizador e invocar `/prumo-vault-policy` com as flags certas:
...
- Para policies do `prumo-secops` (7 AppRoles consolidadas em `secops/vault-policies.hcl`), prefere acrescentar lá em vez de criar um ficheiro novo isolado — questão de single-source.
