#!/usr/bin/env bash
# wire-base · PreToolUse · audit-guard
#
...
#   - existe ${HOME}/.wire/audit-active (marker file criado pela skill antes da Fase 4)
#   - WIRE_AUDIT_ACTIVE=1
#
# Bloqueia (em prod, warn em dev, bypass em lab) quando o tool call é destrutivo E
# `WIRE_AUDIT_APPLY=1` NÃO está definido. Quando `apply` está definido, deixa passar
...
HELPER="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}/lib/wire-common.sh"
if [ -r "$HELPER" ]; then
...
# ────────────────────────────────────────────────────────────────────────────
if [ ! -f "${HOME}/.wire/audit-active" ] && [ "${WIRE_AUDIT_ACTIVE:-}" != "1" ]; then
  exit 0
fi
...
# ────────────────────────────────────────────────────────────────────────────
if [ "${WIRE_AUDIT_APPLY:-}" = "1" ]; then
  wire_log "$PLUGIN" "$HOOK" "audit-apply approved · pass-through"
  exit 0
fi
...
if ! command -v jq >/dev/null 2>&1; then
  wire_log "$PLUGIN" "$HOOK" "jq não disponível · audit-guard inactivo"
  exit 0
fi
...
Para destrancar:
  ...
  2. Se aprovado, exportar WIRE_AUDIT_APPLY=1 antes do tool call.
  3. Para sair de contexto de audit: rm -f ~/.wire/audit-active."
    fi
...
o utilizador tenha confirmado individualmente. Definir WIRE_AUDIT_APPLY=1 só após
...
similares. Para autorizar este edit individualmente:
  ...
  2. Exportar WIRE_AUDIT_APPLY=1 antes do tool call autorizado."
    fi
    ;;
esac
