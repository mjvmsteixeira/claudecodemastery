#!/usr/bin/env bash
# SessionStart hook (wiremaze-base · vault-toolkit)
#
...
#   WMZ_VAULT_AUTO_UP=1     Em modo docker, arranca container se estiver down
#                           (default em dev/lab; off em prod a menos que set)
#   WMZ_OPERATING_MODE      prod | dev | lab — lido por wmz-common.sh
#
# Integração com wiremaze-secops · lifecycle complementar:
...
if [ "$VAULT_MODE" = "docker" ]; then
  ...
  if [ "$STATUS" != "running" ]; then
    # Container ausente ou parado. Em dev/lab (ou com WMZ_VAULT_AUTO_UP=1)
...
if vault_ready; then
  ...
  emit "Vault is sealed and auto-unseal failed. Run: cd \$VAULT_HOME && ./ops-vault.sh unseal — or run /wiremaze-vault-doctor for full diagnosis (if wiremaze-secops is installed)."
fi
